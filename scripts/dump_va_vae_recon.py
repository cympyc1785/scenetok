"""Encode + decode 10 re10k test scenes through VA-VAE at two resolutions
and dump per-scene PNGs under tmp/va_vae_recon/ for visual comparison with
the parallel Wan dump (tmp/wan_vae_recon/).

VA-VAE expects 4D input (B, C, H, W) normalized to [-1, 1] (same convention as
the [0,1] → x*2-1 step in the training path). Latent is f=16, 32 channels.
"""
...
OUT_DIR = REPO_ROOT / "tmp" / "va_vae_recon"
CKPT = "checkpoints/vavae-imagenet256-f16d32-dinov2.pt"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

OUT_DIR.mkdir(parents=True, exist_ok=True)

print(f"Loading VA-VAE from {CKPT}  (device={DEVICE})")
vae = AutoencoderVA(cfg=None).from_pretrained(CKPT)
...
for i, scene in enumerate(scene_keys):
    ...
    for hw in RESOLUTIONS:
        ...
        x = (x01 * 2 - 1).unsqueeze(0).to(DEVICE)        # (1, 3, H, W) in [-1,1]
        z = vae.encode(x).latent_dist.mode().float()     # (1, 32, h, w) deterministic
        y = vae.decode(z.to(x.dtype)).sample             # (1, 3, H, W)
        y01 = ((y[0].float() + 1) / 2).clamp(0, 1).cpu()
