config=custom/lagernvs_va-wan_dl3dv_recon_sceneLRnorm_geoconn_shuf
exp_name="lagernvs_va-wan_dl3dv_recon_sceneLRnorm_geoconn_shuf"
: "${WANDB_API_KEY:?set WANDB_API_KEY in env before running}"
export DEBUG=1
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-2} python -m src.main +experiment=${config} \
