#!/usr/bin/env bash
# Recon variant of train_ti2vgen_dynamicverse_controlnet_dynamic.sh:
#   * context AND target = inpaint_result.mp4 (default video_name, background only)
#     → no dynamic foreground learning, just reconstruction.
#   * text = "" (prompt_style window_dict + missing prompt_file + require_prompt=false)
#     → no text guidance.
# Same controlnet routing (scene + camera), LoRA off. GPU 2.

config=custom/scenetok_va-wan-ti2v_dynamicverse
num_workers=4
gpus=1
num_nodes=1
exp_name="va-wan-ti2v_dynamicverse_recon_controlnet_scene_camera_2_no_lora_no_text"
...
wandb_tags='[dynamicverse,wan-ti2v,controlnet,scene+camera,recon,no_text,inpaint_result_only]'

export WANDB_API_KEY=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
export DEBUG=1

CUDA_VISIBLE_DEVICES=2 exec -a recon_no_text_scenetok_lets_go python -m src.main +experiment=${config} \
