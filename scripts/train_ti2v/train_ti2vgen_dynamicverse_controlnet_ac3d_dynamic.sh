#!/usr/bin/env bash
# AC3D paper VDiT-CC variant of train_ti2vgen_dynamicverse_controlnet_dynamic.sh.
# 같은 데이터/condition routing이지만 camera_input_type 만 controlnet → controlnet_ac3d.
#   * Noise는 main DiT에만, ctrl block 입력 = FC(main_x) + cam_emb + ctrl_x_{prev}
#   * Ctrl block 내부엔 text cross-attn 없음 (self_attn + scene_cross_attn + FFN)
#   * Ctrl → main 잔차는 zero-init ac3d_projectors가 게이팅
# GPU 3, screen train1에서 실행.

config=custom/scenetok_va-wan-ti2v_dynamicverse
num_workers=4
gpus=1
num_nodes=1
exp_name="va-wan-ti2v_dynamicverse_dynamic_controlnet_ac3d_scene_camera_2"
...
wandb_tags='[dynamicverse,wan-ti2v,controlnet_ac3d,scene+camera,lora,video_input,category]'

export WANDB_API_KEY=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
export DEBUG=1

CUDA_VISIBLE_DEVICES=3 exec -a ac3d_scenetok_lets_go python -m src.main +experiment=${config} \
