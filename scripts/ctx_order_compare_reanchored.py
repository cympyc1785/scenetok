"""RE-ANCHORED context-order invariance test (isolates pure set-invariance).

Unlike ctx_order_compare_both.py (target frame fixed to context[0]), here the
target is RE-ANCHORED to the NEW first context view per permutation, so all orders
render the SAME WORLD viewpoint and the reference-frame mismatch is removed.
  - SceneTok: permute context (latent,extrinsics,intrinsics,index) BEFORE
    preprocess(index=0) → anchor = new first view; target re-expressed rel. to it.
  - LagerNVS: permute context IMAGES *and* context_c2w together → worker first_inv
    re-anchors target to the new first view (world viewpoint preserved).

Columns: [context | original | reverse | shuffle]. Rows: top=SceneTok, bottom=
LagerNVS. Output: results/ctx_order_compare_reanchored/<scene>/cmp_2x4.{mp4,gif}
"""
import json
import os
import subprocess
...
OUT = REPO / f"results/ctx_order_compare_reanchored/{SCENE}"
...
def render_scenetok(orders, bnd):
    ...
    for name, order in orders.items():
        ...
        # RE-ANCHOR: permute context BEFORE preprocess so index=0 (anchor) becomes
        # the new first view → target re-expressed relative to it (same world view).
...
def render_lagernvs(orders, bnd):
    ...
    for name, order in orders.items():
        ...
        # RE-ANCHOR: permute context_c2w together with images → worker's first_inv
        # anchors target to the new first view (world viewpoint preserved).
        torch.save({"context_image_paths": [img_paths[i] for i in order],
                    "context_c2w": ctx_c2w[order], "target_c2w": tgt_c2w,
