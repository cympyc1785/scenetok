...
def train(cfg_dict: DictConfig):
    ...
    if cfg.mode == "train" and checkpoint_path is not None and not resume:
        ...
        model_wrapper = SceneWrapper.load_from_checkpoint(checkpoint_path, **kwargs)
    else:
        model_wrapper = SceneWrapper(**kwargs)
    ...
    if cfg.mode == "train":
        trainer.fit(model_wrapper, train_dataloaders=train_dataloaders, val_dataloaders=val_dataloaders["standard"], ckpt_path=checkpoint_path if resume else None)
    elif cfg.mode == "val":
