...
from ...encodings.embeddings import RotaryEmbedding3D
...
class TimestepEmbedder(nn.Module):
    ...
    def forward(self, t: torch.Tensor, pemb: torch.Tensor | None = None) -> torch.Tensor:
        ...
        t_freq = self.timestep_embedding(t, self.frequency_embedding_size)[:, None]
        if pemb is not None:
...
class LitDiT(nn.Module):
    ...
    def forward(
        ...
        x: (N, V, C, H, W) tensor of spatial inputs (images or latent representations of images)
        ...
        p = rearrange(p, "b v n c -> (b v) n c", v=v)
        t = rearrange(t, "b v -> (b v)")
        t = self.t_embedder(t, pemb=p)  
        t = rearrange(t, "(b v) n d -> b (v n) d", v=v)
