config=custom/scenetok_va-wan_lognorm_re10k_scratch
num_workers=8
gpus=1
num_nodes=1
exp_name="scenetok_va-wan_re10k_256_scratch"
...
CUDA_VISIBLE_DEVICES=0 python -m src.main +experiment=${config} \
  ...
  model.compressor.kl_weights=[1e-10,1e-5] \
  +model.compressor.kl_schedule=[0,2000] \
